...
import { Text, View, Button, SafeAreaView, Image, Pressable, TouchableOpacity, ScrollView, Dimensions } from 'react-native';
...
const Suggestions = ({ navigation }) => {
  ...
  const _renderItem = ({item, index}) => {
    return (
      <TouchableOpacity onPress={() => {navigation.navigate('PlantDetail', {plantId: item.id})}}>
        <View style={{borderRadius: 20, height: 356, width: 270, marginLeft: 25, marginTop: 12, backgroundColor: '#FFFFFF', shadowColor: '#000000', shadowOffset: {width: 2, height: 2}, shadowOpacity: 0.12, shadowRadius: 8}}>
          <Image source={item.image} />
          <Text style={{fontFamily: 'QuickSandBold', fontSize: 20, marginLeft: 20, marginTop: 12, color: '#827344'}}>{item.title}</Text>
  ...
  return (
    <ScrollView>
      <View style={{ flex: 1, minHeight: Dimensions.get('window').height, alignItems: 'center', justifyContent: 'flex-start', backgroundColor: '#FFFFFF', paddingTop: 60, paddingBottom: 28}}>
        ...
        <Pressable style={{borderRadius: 50, borderWidth: 1, borderColor: '#DDDDDD', padding: 14, width: 270, backgroundColor: '#827344', position: 'absolute', bottom: 16}} onPress={() => { navigation.navigate('PetFriendly', { lightDir: captured }) }} >
